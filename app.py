...
if st.button('Predict Probability'):
    ...
    # Create a sample pipeline for feature names checking
    ...
    # Fit the sample pipeline to a sample DataFrame
    sample_data = pd.DataFrame({'batting_team': [batting_team], 'bowling_team': [bowling_team], 'city': [selected_city],
                                'runs_left': [runs_left], 'balls_left': [balls_left],
                                'wickets': [remaining_wickets], 'total_runs_x': [target], 'crr': [crr], 'rrr': [rrr]})

    sample_pipeline.fit(sample_data)

    # Get the feature names generated by the ColumnTransformer
    feature_names = sample_pipeline.named_steps['column_transformer'].get_feature_names_out(
        input_features=['batting_team', 'bowling_team', 'city', 'runs_left', 'balls_left', 'wickets', 'total_runs_x', 'crr', 'rrr']
    )
